...
function initDb() {
  ...
  const migrations = [
    "ALTER TABLE wallets ADD COLUMN wallet_type TEXT NOT NULL DEFAULT 'cash'",
    "ALTER TABLE wallets ADD COLUMN credit_limit REAL NOT NULL DEFAULT 0.00",
    ...
    "ALTER TABLE betslips ADD COLUMN cashout_value REAL DEFAULT 0.00",
    "ALTER TABLE betslips ADD COLUMN cashed_out_at TEXT",
    "ALTER TABLE bets ADD COLUMN bet_on TEXT NOT NULL DEFAULT 'win'",
    "ALTER TABLE bets ADD COLUMN cashout_value REAL DEFAULT 0.00",
    `CREATE TABLE IF NOT EXISTS selection_price_history (
      id TEXT PRIMARY KEY,
      selection_id TEXT NOT NULL,
      price_type TEXT NOT NULL DEFAULT 'win',
  ...
  for (const sql of migrations) {
    try { database.exec(sql); } catch(e) {
      // Ignore "duplicate column" and "already exists" errors — expected on re-run
      if (!e.message.includes('duplicate column') && !e.message.includes('already exists')) {
        console.warn('Migration skipped:', e.message.split('\n')[0]);
      }
    }
  }

  // Migrate old 'odds' column to 'win_odds' if needed
  try {
    const cols = database.prepare("PRAGMA table_info(selections)").all().map(c => c.name);
    if (cols.includes('odds') && !cols.some(c => c === 'win_odds' && database.prepare("SELECT win_odds FROM selections LIMIT 1").get()?.win_odds)) {
      database.exec("UPDATE selections SET win_odds = odds WHERE win_odds IS NULL OR win_odds = 0");
      database.exec("UPDATE selections SET opening_win_odds = odds WHERE opening_win_odds IS NULL OR opening_win_odds = 0");
    }
  } catch(e) { /* ignore */ }
